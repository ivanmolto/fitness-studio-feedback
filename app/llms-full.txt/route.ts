...
const llmsFullTxt = `# Fitness Studio Feedback API
...
\`\`\`bash
curl "http://localhost:3000/api/feedback?courseSlug=relax-with-meditation"
\`\`\`
...
[
  {
    "id": "fb-001",
    "courseSlug": "relax-with-meditation",
    "lessonSlug": "breathing-techniques",
    "rating": 5,
    "comment": "This changed everything.",
    "author": "Priya Sharma",
...
curl -X POST "http://localhost:3000/api/feedback" \\
  ...
  -d '{
    ...
    "lessonSlug": "breathing-techniques",
    "rating": 5,
    "comment": "The technique was incredibly helpful.",
    "author": "Alex Turner"
  }'
\`\`\`
...
\`\`\`json
{
  ...
  "courses": [
    ...
    {
      "courseSlug": "breathwork",
      "totalEntries": 4,
      "averageRating": 4.0
    },
    {
      "courseSlug": "yoga-basics",
      "totalEntries": 1,
      "averageRating": 4.0
    }
  ]
...
1. \`GET /api/feedback/summary?courseSlug=relax-with-meditation\`: check the average rating and total entries
2. \`GET /api/feedback?courseSlug=relax-with-meditation&minRating=1\`: pull all low-rated entries
...
3. \`GET /api/feedback/summary?courseSlug=relax-with-meditation\`: check updated stats for the course
 
### Compare feedback across courses
 
1. \`GET /api/feedback/summary\`: get aggregate stats for all courses
2. \`GET /api/feedback?courseSlug=relax-with-meditation\`: pull all feedback for the top-rated course
3. \`GET /api/feedback?courseSlug=breathwork\`: pull all feedback for comparison
`;
